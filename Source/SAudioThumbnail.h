...
/**
    Extended version of AudioThumbnail

    Can be reversed

    @see AudioThumbnailCache, AudioThumbnailBase, AudioThumbnail

    @tags{Audio}
*/
class SAudioThumbnail : public AudioThumbnailBase
{
public:
    //==============================================================================
    ...
    */
    SAudioThumbnail (int sourceSamplesPerThumbnailSample,
                    AudioFormatManager& formatManagerToUse,
                    AudioThumbnailCache& cacheToUse);

    /** Destructor. */
    ~SAudioThumbnail() override;
...
private:
    ...
    class SLevelDataSource;
    struct SMinMaxValue;
    class SThumbData;
    class SCachedWindow;

    std::unique_ptr<SLevelDataSource> source;
    std::unique_ptr<SCachedWindow> window;
    OwnedArray<SThumbData> channels;
    ...
    bool setDataSource (SLevelDataSource* newSource);
    void setLevels (const SMinMaxValue* const* values, int thumbIndex, int numChans, int numValues);
    void createChannels (int length);

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SAudioThumbnail)
};
