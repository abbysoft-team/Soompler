...
struct SAudioThumbnail::SMinMaxValue
{
    SMinMaxValue() noexcept
    {
        values[0] = 0;
        values[1] = 0;
    }
...
//==============================================================================
class SAudioThumbnail::SLevelDataSource   : public TimeSliceClient
{
public:
    SLevelDataSource (SAudioThumbnail& thumb, AudioFormatReader* newReader, int64 hash)
        : hashCode (hash), owner (thumb), reader (newReader)
    {
    }

    SLevelDataSource (SAudioThumbnail& thumb, InputSource* src)
        : hashCode (src->hashCode()), owner (thumb), source (src)
    {
    }

    ~SLevelDataSource() override
    {
    ...
    bool readNextBlock()
    {
        ...
        if (! isFullyLoaded())
        {
            ...
            if (numToDo > 0)
            {
                ...
                HeapBlock<SMinMaxValue> levelData ((unsigned int) numThumbSamps * numChannels);
                HeapBlock<SMinMaxValue*> levels (numChannels);
...
//==============================================================================
class SAudioThumbnail::SThumbData
{
public:
    SThumbData (int numThumbSamples)
    {
        ensureSize (numThumbSamples);
    }

    inline SMinMaxValue* getData (int thumbSampleIndex) noexcept
    {
    ...
    void getMinMax (int startSample, int endSample, SMinMaxValue& result) const noexcept
    {
        if (startSample >= 0)
        {
    ...
    void write (const SMinMaxValue* values, int startIndex, int numValues)
    {
        resetPeak();
...
private:
    ...
    void ensureSize (int thumbSamples)
    {
        ...
        if (extraNeeded > 0)
            data.insertMultiple (-1, SMinMaxValue(), extraNeeded);
    }
};

//==============================================================================
class SAudioThumbnail::SCachedWindow
{
public:
    SCachedWindow() {}
    ...
    void drawChannel (Graphics& g, const Rectangle<int>& area,
                      ...
                      SLevelDataSource* levelData, const OwnedArray<SThumbData>& chans)
    {
...
private:
                ...
                if (sample >= 0)
                {
                    ...
                    else
                    {
                        ...
                        for (int chan = 0; chan < totalChans; ++chan) {
                            if (reversed) {
                                getData (chan, numSamples - i - 1)->setFloat (levels.getReference (chan));
                            } else {
    ...
    SMinMaxValue* getData (const int channelNum, const int cacheIndex) noexcept
    {
    ...
    void ensureSize (const int numSamples)
    {
        ...
        if (data.size() < itemsRequired)
            data.insertMultiple (-1, SMinMaxValue(), itemsRequired - data.size());
    }
};

//==============================================================================
SAudioThumbnail::SAudioThumbnail (const int originalSamplesPerThumbnailSample,
                                AudioFormatManager& formatManager,
                                AudioThumbnailCache& cacheToUse)
    : formatManagerToUse (formatManager),
      cache (cacheToUse),
      window (new SCachedWindow()),
      samplesPerThumbSample (originalSamplesPerThumbnailSample)
{
}

SAudioThumbnail::~SAudioThumbnail()
{
    clear();
}

void SAudioThumbnail::clear()
{
    source.reset();
...
void SAudioThumbnail::clearChannelData()
{
    window->invalidate();
    channels.clear();
...
void SAudioThumbnail::reset (int newNumChannels, double newSampleRate, int64 totalSamplesInSource)
{
    clear();
...
void SAudioThumbnail::createChannels (const int length)
{
    while (channels.size() < numChannels)
        channels.add (new SThumbData (length));
}

//==============================================================================
bool SAudioThumbnail::loadFrom (InputStream& rawInput)
{
...
void SAudioThumbnail::saveTo (OutputStream& output) const
{
    const ScopedLock sl (lock);
...
//==============================================================================
bool SAudioThumbnail::setDataSource (SLevelDataSource* newSource)
{
...
bool SAudioThumbnail::setSource (InputSource* const newSource)
{
    clear();

    return newSource != nullptr && setDataSource (new SLevelDataSource (*this, newSource));
}

void SAudioThumbnail::setReader (AudioFormatReader* newReader, int64 hash)
{
    clear();

    if (newReader != nullptr)
        setDataSource (new SLevelDataSource (*this, newReader, hash));
}

int64 SAudioThumbnail::getHashCode() const
{
    return source == nullptr ? 0 : source->hashCode;
}

void SAudioThumbnail::reverse()
{
    if (window != nullptr) {
        window->reverse();
    }
                               ...
                               int startOffsetInBuffer, int numSamples)
{
    ...
    if (numToDo > 0)
    {
        ...
        const HeapBlock<SMinMaxValue> thumbData (numToDo * numChans);
        const HeapBlock<SMinMaxValue*> thumbChannels (numChans);
...
void SAudioThumbnail::setLevels (const SMinMaxValue* const* values, int thumbIndex, int numChans, int numValues)
{
    const ScopedLock sl (lock);
...
//==============================================================================
int SAudioThumbnail::getNumChannels() const noexcept
{
    return numChannels;
}

double SAudioThumbnail::getTotalLength() const noexcept
{
    return sampleRate > 0 ? (totalSamples / sampleRate) : 0;
}

bool SAudioThumbnail::isFullyLoaded() const noexcept
{
    return numSamplesFinished >= totalSamples - samplesPerThumbSample;
}

double SAudioThumbnail::getProportionComplete() const noexcept
{
    return jlimit (0.0, 1.0, numSamplesFinished / (double) jmax ((int64) 1, totalSamples));
}

int64 SAudioThumbnail::getNumSamplesFinished() const noexcept
{
    return numSamplesFinished;
}

float SAudioThumbnail::getApproximatePeak() const
{
    const ScopedLock sl (lock);
    int peak = 0;
...
void SAudioThumbnail::getApproximateMinMax (double startTime, double endTime, int channelIndex,
...
void SAudioThumbnail::drawChannel (Graphics& g, const Rectangle<int>& area, double startTime,
...
void SAudioThumbnail::drawChannels (Graphics& g, const Rectangle<int>& area, double startTimeSeconds,
